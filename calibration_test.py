...
def showUndistortionResults():
    ...
    cv2.imshow('original displayL', grayL)
...
while photo_counter != total_photos:
    ...
    if (leftExists and rightExists):
        ...
        '''if ((retL == True) and (retR == True)) and (img_height <= photo_height):
            ...
            print(
                "Image resolution is higher than photo resolution, upscale needed. Please check your photo and image parameters!")
            exit(0)'''
...
objpointsLeft = np.array(objpointsLeft)
...
np.savez('./calibration_data/{}p/camera_calibration_{}.npz'.format(img_height, 'left'),
         map1=leftMapX, map2=leftMapY, objpoints=objpointsLeft, imgpoints=imgpointsLeft,
         camera_matrix=leftCameraMatrix, distortion_coeff=leftDistortionCoefficients)

rightMapX, rightMapY = cv2.initUndistortRectifyMap(
    rightCameraMatrix, rightDistortionCoefficients, rightRectification,
    rightProjection, image_size, cv2.CV_32FC1)
np.savez('./calibration_data/{}p/camera_calibration_{}.npz'.format(img_height, 'right'),
         map1=rightMapX, map2=rightMapY, objpoints=objpointsRight, imgpoints=imgpointsRight,
         camera_matrix=rightCameraMatrix, distortion_coeff=rightDistortionCoefficients)

if not os.path.isdir('./calibration_data'):
    os.mkdir('./calibration_data')
np.savez_compressed('./calibration_data/480p/stereo_camera_calibration.npz',
